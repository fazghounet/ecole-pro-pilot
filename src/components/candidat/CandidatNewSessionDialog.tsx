...
export default function CandidatNewSessionDialog() {
  ...
  const [form, setForm] = useState<NewSessionForm>({ type: "", date: null, hour: "" });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.type || !form.date || !form.hour) {
      setSuccess(false);
      return;
    }
    setSuccess(true);
    // Ici, on pourrait lancer une requête API
    setTimeout(() => {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium mb-1" htmlFor="type">Type de séance</label>
          ...
          <div>
            <label className="block text-sm font-medium mb-1" htmlFor="hour">Heure</label>
            <Input
              id="hour"
              type="time"
              value={form.hour}
          ...
          {success && (
            <div className="text-green-600 text-sm">Votre demande a été envoyée !</div>
          )}
          <DialogFooter>
            <DialogClose asChild>
